/* BRIEF Choose 1 from the following 3 artworks, and recreate the work using p5.js. You must credit the original artist in your code/documentation with artist name, title of work, and date. 
...
/* By Beatriz Lopez Neira, October 2021
I will be recreating Concreto by Alfredo Volpi, December 1950 */
...
function draw() {
	...
	var widthStep = width/8; //Defining width step
	var heightStep = height/11; //Defining height step
	let green = color(0,102,102); 
	let cream = color(218,210,160);
  ...
  noStroke(); //Eliminating the default stroke for all triangles 
  ...
  for (var i = 0; i < 7; i++){
    if (i<6 && i%2==0){
      ...
      triangle(x1,y1,x2,y2,x3,y3); 
			fill(cream)
			triangle(x1+ widthStep,y1+heightStep,x2 + widthStep,y2+heightStep,x3 + widthStep,y3+heightStep);
			fill(cream)
			triangle(x1+ 2*widthStep,y1+heightStep,x2 + 2*widthStep,y2+heightStep,x3 + 2*widthStep,y3+heightStep);
